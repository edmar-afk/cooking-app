		...
		<>
			{foods.map((food) => (
				<Link to={`/recipe/${food.id}`}
					...
					<h3 className="z-10 mt-3 text-xl font-bold text-white">{food.name}</h3>
					<p className="z-10 text-white text-xs mr-4 flex flex-row items-center">{food.title}</p>
					<div className="z-10 flex flex-row mt-3 text-xs leading-6 text-gray-300">
